...
namespace CubaseDrumMapEditor.Models
{
    public static class MidiUtility
    {
        ...
        {
            if (NoteNameToNumberDict.TryGetValue(noteName, out int noteNumber))
            {
                return noteNumber;
            }
        ...
        private static readonly Dictionary<string, int> NoteNameToNumberDict = new Dictionary<string, int>
        {
            {"C-2", 0},
            {"C#-2", 1},
            {"D-2", 2},
        ...
        private static readonly Dictionary<int, string> NoteNumberToNameDict = NoteNameToNumberDict.ToDictionary(kvp => kvp.Value, kvp => kvp.Key);
